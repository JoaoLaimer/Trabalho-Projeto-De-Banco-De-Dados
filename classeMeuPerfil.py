...
class MeuPerfilPage:
    def __init__(self, master, id_user, id_user_logged, app):
        ...
        if id_user_logged == id_user:
            ...
            user_alter_password = tk.Button(self.app, text="Alternar Senha", command=self.change_password)
            user_alter_password.grid(row=3, column=0, padx=10, pady=10)
        else:
            user_my_list = tk.Button(self.app, text="Lista de Filmes", command=lambda: self.master.exibir_lista_filmes(id_user, False))
            user_my_list.grid(row=2, column=0, padx=10, pady=10)
